...
#     padding_num = 7
#
#     new_obs, new_pred, new_obs_rel, new_pred_rel, new_ctx, new_priors = [], [], [], [], [], []
#
#     for i in range(len(data)):
#         agent_num = obs_seq_list[i].shape[0]
#         idx_list = list(range(agent_num)) * padding_num
#         idx_list = idx_list[:padding_num]  # 简单截断或重复
#
#         def get_padded(tensor_list, idxs):
#             return torch.stack([tensor_list[idx] for idx in idxs], dim=0)
#
#         new_obs.append(get_padded(obs_seq_list[i], idx_list))
#         new_pred.append(get_padded(pred_seq_list[i], idx_list))
#         new_obs_rel.append(get_padded(obs_seq_rel_list[i], idx_list))
#         new_pred_rel.append(get_padded(pred_seq_rel_list[i], idx_list))
#         new_ctx.append(get_padded(context_list[i], idx_list))
#
#         if has_priors:
#             new_priors.append(get_padded(priors_list[i], idx_list))
#
#     # Stack
...
#     seq_start_end = torch.tensor([1] * len(data))  # Dummy
#
...
def seq_collate_with_padding(data):
    ...
    Return 7 elements if priors exist, else 6.
    """
    # 检查是否包含 priors
    has_priors = (len(data[0]) > 5)
    ...
    if has_priors:
        priors_traj = torch.stack(new_priors, dim=0)
        results.append(priors_traj)
        # 返回: (obs, pred, obs_rel, pred_rel, context, seq_start, priors)
...
def get_batch_route_priors(obs_traj, rag_system, embedder, k_retrieve=20, n_clusters=3, pred_len=12, device='cuda'):
    """
    输入: obs_traj (Batch, Agents, 3, Obs_Len)
    输出: priors (Batch, Agents, n_clusters, pred_len, 4)
    """
    if obs_traj.shape[2] == 3 and obs_traj.shape[3] != 3:
        obs_traj = obs_traj.permute(0, 1, 3, 2)
