...
class Data:
    ...
    def read_weather(self):
        ...
        self.weather = pd.read_csv(self.weather_path, low_memory=False)  # 针对天气数据
# 若轨迹数据读取也有警告，同样添加：
# trajectory_data = pd.read_csv(trajectory_path, low_memory=False)
        self.weather['datetime'] = pd.to_datetime(self.weather['valid'],format="%Y-%m-%d %H:%M")
#         print(self.weather)

    ## 主处理循环，遍历self.filelist中的每个CSV轨迹文件
            ...
            with open(i, mode='r') as csv_file:
                ...
                for row in csv_reader:
                    if row["ID"] is not "" and  row["Range"] is not None and row["Bearing"] is not None :
                        ID = row["ID"]
                        Range = row["Range"]  #距离
                        Bearing = row["Bearing"] #方向角
                        Altitude = row["Altitude"] #高度
                        k = Range + Bearing + ID
